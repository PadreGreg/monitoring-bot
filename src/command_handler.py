...
class CommandHandler:
    ...
    async def cmd_list_alert_channels(self, event, args):
        ...
        try:
            ...
            # Format channels list
            channels_list = "\n".join([
                f"• {channel['channel_id']} {'(Primary)' if channel['is_primary'] else ''}"
                for channel in channels
            ])
            message = f"🔔 **Alert Channels** ({len(channels)}):\n\n{channels_list}"
            
            await event.reply(message)
        except Exception as e:
            logger.error(f"Error in list_alert_channels command: {e}")
            await event.reply(f"⚠️ Error getting alert channels: {str(e)}")
